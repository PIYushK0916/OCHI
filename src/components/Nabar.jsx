...
const Navbar = () => {
  ...
  return (
    <div className={`fixed top-0 left-0 w-full px-20 py-8 font-['Neue Montreal'] flex justify-between z-[999] transition-transform duration-300 ease-in-out backdrop-blur-md ${scrollDirection === 'down' ? '-translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}>
      <div className="logo">
      ...
      <div className="links flex gap-10">
        {['Services', 'Our Work', 'About us', 'Insights', 'contact us'].map((items, index) => (
          <a href="" key={index} className={`text-xl font-['Neue Montreal'] capitalize font-regular ${index === 4 && "ml-32"} `}>{items}</a>
        ))}
      </div>
    </div>
  );
